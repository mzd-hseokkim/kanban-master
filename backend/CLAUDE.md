...
### ⚠️ CRITICAL: Transaction Boundaries with Spring Data JPA Pagination

**This is the #1 source of LazyInitializationException in pagination scenarios. Get this wrong and you get:**
```
LazyInitializationException: could not initialize proxy [com.kanban.user.User#2] - no Session
```

#### The Problem

When using `Page<Entity>` with pagination, converting entities to DTOs using `Page.map()` **MUST** happen INSIDE the `@Transactional` service method. If you do it in the controller (outside transaction), the Hibernate Session closes and lazy-loaded relationships fail.

#### ❌ INCORRECT PATTERN (Causes LazyInitializationException)

```java
// ❌ WRONG - Controller tries to convert outside transaction
@RestController
public class MemberController {
    ...
    public ResponseEntity<Page<MemberResponse>> getMembers(
        ...
        Page<BoardMember> members = memberService.getMembers(boardId, pageable);
        // ERROR! Session is closed here, lazy-loaded relationships fail
        return ResponseEntity.ok(members.map(MemberResponse::from));
    }
}

// ❌ WRONG - Service returns raw Page<Entity>
@Service
@RequiredArgsConstructor
@Transactional(readOnly = true)
public class MemberService {
    ...
    public Page<BoardMember> getMembers(Long boardId, Pageable pageable) {
        // Returns raw entity page, conversion happens outside @Transactional
        return repository.findByBoardIdOrderByCreatedAtAsc(boardId, pageable);
    }
}
```
...
```java
// ✅ CORRECT - Controller uses converted DTO Page
@RestController
public class MemberController {
    private final MemberService memberService;
    ...
    public ResponseEntity<Page<MemberResponse>> getMembers(
        ...
        // Service already converted to DTO, safe to use
        Page<MemberResponse> responses = memberService.getMembersResponse(boardId, pageable);
        return ResponseEntity.ok(responses);
    }
}

// ✅ CORRECT - Service converts Page<Entity> to Page<DTO> inside @Transactional
@Service
@RequiredArgsConstructor
...
public class MemberService {
    ...
    // Return DTO Page, NOT Entity Page
    public Page<MemberResponse> getMembersResponse(Long boardId, Pageable pageable) {
        Page<BoardMember> members = repository.findByBoardIdOrderByCreatedAtAsc(boardId, pageable);

        // Page.map() happens INSIDE @Transactional context
        // Session is still open, lazy-loaded relationships work fine
        return members.map(MemberResponse::from);
    }

    // Optional: For cases where you need the raw entity page
    public Page<BoardMember> getMembers(Long boardId, Pageable pageable) {
...
2. `@Transactional` annotation opens Hibernate Session
3. Service queries database → loads proxies for lazy relationships
4. Service calls `Page.map()` → proxies are resolved (Session is open ✅)
5. Service returns DTO page
6. `@Transactional` closes Session
7. Controller receives DTO page (relationships already loaded ✅)

**What Fails**:
```java
// If you convert outside @Transactional:
1. Service returns Page<Entity>
2. @Transactional closes Session  ← ⚠️ Session is now closed
3. Controller calls map()
4. map() tries to access lazy-loaded relationship
...
public interface BoardMemberRepository extends JpaRepository<BoardMember, BoardMemberId> {
    ...
    // ✅ For pagination - Spring Data JPA handles JOIN FETCH properly with Page
    @Query("SELECT bm FROM BoardMember bm JOIN FETCH bm.user WHERE bm.board.id = :boardId ORDER BY bm.createdAt ASC")
    Page<BoardMember> findByBoardIdOrderByCreatedAtAsc(
        @Param("boardId") Long boardId,
        Pageable pageable);

    // ✅ For list queries - Can use LEFT JOIN FETCH if needed
    @Query("SELECT bm FROM BoardMember bm JOIN FETCH bm.board JOIN FETCH bm.user WHERE bm.user.id = :userId AND bm.invitationStatus = :status ORDER BY bm.invitedAt DESC")
    List<BoardMember> findPendingInvitationsByUserId(
...
**Key Points**:
- Spring Data JPA **automatically handles** JOIN FETCH with Pagination (since Spring 6.1)
- Don't use `LEFT JOIN FETCH` with pagination (it can cause issues)
- Use `JOIN FETCH` to eagerly load relationships when needed
- For list queries (not paginated), you can use `LEFT JOIN FETCH`
...
public class MemberService {
    ...
    // For paginated responses - always return DTO Page
    public Page<MemberResponse> getMembersPageResponse(Long boardId, Pageable pageable) {
        Page<BoardMember> members = repository.findByBoardIdOrderByCreatedAtAsc(boardId, pageable);
        return members.map(MemberResponse::from);  // Happens inside @Transactional ✅
    }

    // For list responses - can return entity list if needed, but convert to DTO
    public List<MemberResponse> getPendingInvitations(Long userId) {
        List<BoardMember> invitations = repository
            .findPendingInvitationsByUserId(userId, InvitationStatus.PENDING);
        return invitations.stream()
            .map(MemberResponse::from)
            .toList();  // Happens inside @Transactional ✅
    }
}
```

#### Real-World Example from Kanban Board

**The Bug** (invitation system):
```java
// ❌ WRONG
...
// Then in controller:
return ResponseEntity.ok(members.map(BoardMemberResponse::from));  // ❌ LazyInitializationException
```
...
public Page<BoardMemberResponse> getBoardMembersPageResponse(Long boardId, Pageable pageable) {
    ...
    return members.map(BoardMemberResponse::from);  // ✅ Happens inside @Transactional
}

// Then in controller:
return ResponseEntity.ok(memberService.getBoardMembersPageResponse(boardId, pageable));  // ✅ Works
```

#### Checklist for Pagination APIs

- [ ] Service method is marked `@Transactional(readOnly = true)`
- [ ] Service calls `Page.map()` to convert Entity → DTO
- [ ] Service returns `Page<DTO>`, NOT `Page<Entity>`
- [ ] Controller receives already-converted `Page<DTO>`
- [ ] Repository queries use `JOIN FETCH` (not `LEFT JOIN FETCH`) with pagination
- [ ] All lazy relationships accessed/resolved before returning from service
- [ ] No exceptions about "could not initialize proxy"
