...
public interface BoardRepository extends JpaRepository<Board, Long> {
       ...
       @Query("SELECT b FROM Board b "
                     ...
                     + "ORDER BY b.updatedAt DESC")
       List<Board> findActiveByWorkspaceIdAndOwnerId(@Param("workspaceId") Long workspaceId,
       ...
       @Query("SELECT b FROM Board b "
                     + "WHERE b.workspace.id = :workspaceId AND b.status = 'ACTIVE' "
                     + "ORDER BY b.updatedAt DESC " + "LIMIT :limit")
       List<Board> findRecentActiveBoards(@Param("workspaceId") Long workspaceId,
                     @Param("limit") int limit);

       /**
        * 최근 N개의 활성 보드 조회 (모든 워크스페이스)
        */
       @Query("SELECT b FROM Board b " + "WHERE b.status = 'ACTIVE' " + "ORDER BY b.updatedAt DESC "
                     + "LIMIT :limit")
        ...
        */
       @Query("SELECT b FROM Board b " + "WHERE b.status = 'DELETED' AND b.deletedAt < :cutoffDate")
       ...
       boolean existsByNameInWorkspace(@Param("workspaceId") Long workspaceId,
                     @Param("name") String name);

       /**
        * DTO 형태로 보드 조회 (워크스페이스/소유자 정보 포함) 14개 파라미터 생성자 사용 (Hibernate 6.4+ 호환)
        */
