...
public class MemberService {
        ...
        public BoardMemberResponse inviteMember(Long boardId, Long userId, Long invitedByUserId,
                ...
                Board board = boardRepository.findById(boardId).orElseThrow(
                                () -> new IllegalArgumentException("보드를 찾을 수 없습니다: " + boardId));
                ...
                User invitedByUser = userRepository.findById(invitedByUserId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "사용자를 찾을 수 없습니다: " + invitedByUserId));
                ...
                BoardMember member = BoardMember.builder()
                                .id(BoardMemberId.builder().boardId(boardId).userId(userId).build())
                                .board(board).user(user).role(role)
        ...
        public BoardMemberResponse acceptInvitation(String token) {
                BoardMember member = boardMemberRepository.findByInvitationToken(token).orElseThrow(
                                () -> new IllegalArgumentException("초대 토큰이 유효하지 않습니다"));
        ...
        public BoardMemberResponse changeMemberRole(Long boardId, Long memberId,
                ...
                User changedByUser = userRepository.findById(changedByUserId)
                                .orElseThrow(() -> new IllegalArgumentException(
                                                "사용자를 찾을 수 없습니다: " + changedByUserId));
                ...
                activityService.recordActivity(ActivityScopeType.BOARD, boardId,
                                ...
                                changedByUser.getName() + "님이 " + member.getUser().getName()
                                                + "님의 권한을 변경했습니다 (" + oldRole + " → " + newRole
                                                + ")");

                log.info("Member role changed - Board: {}, User: {}, NewRole: {}", boardId,
                                memberId, newRole);
                return BoardMemberResponse.from(savedMember);
        }
        ...
        public List<BoardMemberResponse> getBoardMembers(Long boardId) {
                ...
                List<BoardMemberResponse> members = new java.util.ArrayList<>(
                                boardMemberRepository.findByBoardIdOrderByCreatedAtAsc(boardId)
                                                .stream().map(BoardMemberResponse::from).toList());

                // 소유자가 멤버 리스트에 있는지 확인
                boolean ownerExists = members.stream()
                                .anyMatch(m -> m.getUserId().equals(board.getOwner().getId()));

                // 소유자가 없으면 리스트 맨 앞에 추가
                if (!ownerExists) {
                        User owner = board.getOwner();
                        BoardMemberResponse ownerResponse = BoardMemberResponse.builder()
                                        .boardId(boardId).userId(owner.getId())
                                        .userEmail(owner.getEmail()).userName(owner.getName())
                                        .role(BoardMemberRole.MANAGER) // Owner is effectively a
                                                                       // manager
                                        .invitationStatus(InvitationStatus.ACCEPTED)
                                        .createdAt(board.getCreatedAt() != null ? board
                                                        .getCreatedAt()
                                                        .format(java.time.format.DateTimeFormatter.ISO_LOCAL_DATE_TIME)
                                                        : null)
                                        .boardName(board.getName()).avatarUrl(owner.getAvatarUrl())
                                        .build();
        ...
        public List<BoardMemberResponse> getBoardAcceptedMembers(Long boardId) {
                return boardMemberRepository
                                .findByBoardIdAndInvitationStatusOrderByCreatedAtAsc(boardId,
         ...
         */
        public Page<BoardMemberResponse> getBoardMembersPageResponse(Long boardId,
                        Pageable pageable) {
                Board board = boardRepository.findById(boardId)
                                .orElseThrow(() -> new IllegalArgumentException("보드를 찾을 수 없습니다"));

                Page<BoardMember> membersPage = boardMemberRepository
                                .findByBoardIdOrderByCreatedAtAsc(boardId, pageable);

                List<BoardMemberResponse> memberResponses = new java.util.ArrayList<>(
                                membersPage.map(BoardMemberResponse::from).getContent());

                // 첫 페이지이고 소유자가 리스트에 없으면 추가
                if (pageable.getPageNumber() == 0) {
                        boolean ownerExists = memberResponses.stream().anyMatch(
                                        m -> m.getUserId().equals(board.getOwner().getId()));
                        ...
                        if (!ownerExists) {
                                ...
                                BoardMemberResponse ownerResponse = BoardMemberResponse.builder()
                                                ...
                                                .avatarUrl(owner.getAvatarUrl()).build();
                                memberResponses.add(0, ownerResponse);
                        }
                }

                long totalElements = membersPage.getTotalElements();
                if (memberResponses.size() > membersPage.getNumberOfElements()) {
                        totalElements++;
                }

                return new org.springframework.data.domain.PageImpl<>(memberResponses, pageable,
                                totalElements);
        }
        ...
        public List<BoardMemberResponse> getUserBoardMemberships(Long userId) {
                return boardMemberRepository
                                .findByUserIdAndInvitationStatus(userId, InvitationStatus.ACCEPTED)
                                .stream().map(BoardMemberResponse::from).toList();
        }
        ...
        public void expireOldInvitations(Long boardId) {
                ...
                for (BoardMember member : expiredMembers) {
                        ...
                        boardMemberRepository.save(member);
                        log.info("Invitation expired - Board: {}, User: {}", boardId,
                                        member.getUser().getId());
                }
        }
